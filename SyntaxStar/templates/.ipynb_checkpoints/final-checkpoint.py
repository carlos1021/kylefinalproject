...
def main(word):
    similar = SimilarWords().load_dataset(wikipedia_query=word, wikipedia_query_limit=5)
    words, _ = similar.find_similar_words(input_context='', input_words=[word])
    print(words)

if __name__ == "__main__":
    word = sys.argv[1] if len(sys.argv) > 1 else None  # Default example
    main(word)